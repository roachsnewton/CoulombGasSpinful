...
def pretrain_spin(van, params_van,
             nup, ndown, dim, Theta, Emax, twist,
             path, key,
    ...
    if dim == 3:
        L = (4/3*jnp.pi*nup)**(1/3)
        beta = 1 / ((4.5*jnp.pi)**(2/3) * Theta)
    elif dim == 2:
        L = jnp.sqrt(jnp.pi*nup)
        beta = 1/ (4 * Theta)
    ...
    for i in range(1, epoch+1):
        ...
        t1 = time.time()
        print("iter: %04d" % i,
                " F: %.6f" % F, "(%.6f)" % (F_std / jnp.sqrt(batch)),
                " E: %.6f" % E, "(%.6f)" % (E_std / jnp.sqrt(batch)),
                " S: %.6f" % S, "(%.6f)" % (S_std / jnp.sqrt(batch)),
                " dt: %.3f" % (t1-t0))
