...
# folder to save data.
parser.add_argument("--folder", default="MLCodes/CoulombGasSpinful-master-ZQ/datas-spin/", 
                    help="the folder to save data")

# physical parameters.
# spin up and spin down should be equal: nup = ndown = n/2 (total spin sz = 0)
parser.add_argument("--nup"  , type=int, default=7, help="total number of electrons")
parser.add_argument("--ndown", type=int, default=7, help="total number of electrons")

# dimensional
parser.add_argument("--dim", type=int, default=3, help="spatial dimension")
parser.add_argument("--rs", type=float, default=1.0, help="rs")
parser.add_argument("--Theta", type=float, default=0.50, help="dimensionless temperature T/Ef")

# twist angle
# dim = 2, twist = [1/4, 1/4]
# dim = 3, twist = [1/4, 1/4, 1/4]
# parser.add_argument("--twist", type=float, nargs="+", default=[1/4, 1/4], 
# help="(scaled) twist angle in the range [-1/2, 1/2]^dim")

# many-body state distribution: autoregressive transformer.
parser.add_argument("--Emax", type=int, default=40, help="energy cutoff for the single-particle orbitals")
...
print("Total number of many-body states (%dup %ddown in %d): %d" 
      % (nup, ndown, num_states, comb(num_states, nup) + comb(num_states, ndown)))
...
def forward_fn(state_idx):
    ## Transformer input:(num_states, num_layers, model_size, num_heads, hidden_size)
...
freefermion_path = args.folder + "freefermion_pretraining/" \
                + "n_%d_%d_%d_dim_%d_Theta_%.2f_Emax_%d" \
                    % (n, nup, ndown, dim, args.Theta, args.Emax) \
...
if os.path.isfile(pretrained_model_filename):
    ...
    params_van = src.freefermion.pretraining_spin.pretrain_spin(van, params_van,
                          nup, ndown, dim, args.Theta, args.Emax, twist,
                          freefermion_path, key,
                          pre_lr, pre_sr, pre_damping, pre_maxnorm,
                          pre_batch, epoch=2000)
...
def flow_fn(x):
    model = src.flow_spin.FermiNet_spin(args.depth, args.spsize, args.tpsize, L)
    return model(x)
...
path = args.folder + "interacting/" \
                   + "n_%d_%d_%d_dim_%d_rs_%.1f_Theta_%.2f_Emax_%d" \
...
for i in range(args.epoch_finished + 1, args.epoch + 1):
    ...
    print("iter: %04d" % i,
            " F: %.6f" % (F/args.rs**2), "(%.6f)" % (F_std/args.rs**2),
            " E: %.6f" % (E/args.rs**2), "(%.6f)" % (E_std/args.rs**2),
            " K: %.6f" % (K/args.rs**2), "(%.6f)" % (K_std/args.rs**2),
            " V: %.6f" % (V/args.rs**2), "(%.6f)" % (V_std/args.rs**2),
            " S: %.6f" % S,              "(%.6f)" % S_std,
            " acc: %.4f" % accept_rate,
            " dt: %.3f" % (t1-t0))
