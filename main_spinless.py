...
### 
parser.add_argument("--folder", default="MLCodes/CoulombGasSpinful-master-ZQ/datas/", help="the folder to save data")

# physical parameters.
parser.add_argument("--n", type=int, default=9, help="total number of electrons")
parser.add_argument("--dim", type=int, default=2, help="spatial dimension")
parser.add_argument("--rs", type=float, default=5.0, help="rs")
...
# parser.add_argument("--twist", type=float, nargs="+", default=[1/4, 1/4, 1/4], help="(scaled) twist angle in the range [-1/2, 1/2]^dim")
...
for i in range(args.epoch_finished + 1, args.epoch + 1):
    ...
    print("iter: %04d" % i,
            " F: %.6f" % (F/args.rs**2), " F_std: %.6f" % (F_std/args.rs**2),
            " E: %.6f" % (E/args.rs**2), " E_std: %.6f" % (E_std/args.rs**2),
            " K: %.6f" % (K/args.rs**2), " K_std: %.6f" % (K_std/args.rs**2),
            " V: %.6f" % (V/args.rs**2), " V_std: %.6f" % (V_std/args.rs**2),
            " S: %.6f" % S,              " S_std: %.6f" % S_std,
            " acc: %.4f" % accept_rate,
            " dt: %.3f" % (t1-t0))
